...
object StreamSideOutputScala {
  def main(args: Array[String]): Unit = {
    ...
    val outputStream = text.process((value: Int, ctx: ProcessFunction[Int, Int]#Context, _: Collector[Int]) => {
      if (value % 2 == 0) {
    ...
    val subOutputStream = evenStream.process((value: Int, ctx: ProcessFunction[Int, Int]#Context, _: Collector[Int]) => {
      if (value <= 5) {
