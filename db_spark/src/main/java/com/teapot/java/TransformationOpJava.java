...
public class TransformationOpJava {
    ...
    private static void distinctOp(JavaSparkContext sc) {
        ...
        dataRDD.map((Function<Tuple2<Integer, String>, String>) tup -> tup._2)
                .distinct()
                .foreach((VoidFunction<String>) System.out::println);
    }
    ...
    private static void joinOp(JavaSparkContext sc) {
        ...
        JavaPairRDD<Integer, String> dataRDD1Pair = dataRDD1.mapToPair(
                (PairFunction<Tuple2<Integer, String>, Integer, String>) tup -> new Tuple2<>(tup._1, tup._2));

        JavaPairRDD<Integer, Integer> dataRDD2Pair = dataRDD2.mapToPair((PairFunction<Tuple2<Integer, Integer>, Integer, Integer>) tup -> new Tuple2<>(tup._1, tup._2));

        dataRDD1Pair.join(dataRDD2Pair).foreach((VoidFunction<Tuple2<Integer, Tuple2<String, Integer>>>) System.out::println);
    ...
    private static void sortByKeyOp(JavaSparkContext sc) {
        ...
        //使用sortBy
        dataRDD.sortBy((Function<Tuple2<Integer, Integer>, Integer>) tup -> tup._2, false, 1)
                .foreach((VoidFunction<Tuple2<Integer, Integer>>) System.out::println);
    }
    ...
    private static void reduceByKeyOp(JavaSparkContext sc) {
        ...
        JavaRDD<Tuple2<Integer, String>> dataRDD = sc.parallelize(Arrays.asList(t1, t2, t3, t4));
        dataRDD.mapToPair((PairFunction<Tuple2<Integer, String>, String, Integer>) tup -> new Tuple2<String, Integer>(tup._2, 1)).reduceByKey(new Function2<Integer, Integer, Integer>() {
            @Override
    ...
    private static void groupByKeyOp(JavaSparkContext sc) {
        ...
        dataRDD.mapToPair(
                (PairFunction<Tuple2<Integer, String>, String, Integer>) tup -> new Tuple2<>(tup._2, tup._1))
                .groupByKey().
                foreach((VoidFunction<Tuple2<String, Iterable<Integer>>>) tup -> {
                    //获取大区信息
                    String area = tup._1;
    ...
    private static void groupByKeyOp2(JavaSparkContext sc) {
        ...
        dataRDD.mapToPair((PairFunction<Tuple3<Integer, String, String>, String, Tuple2<Integer, String>>) tup -> {
            //("US",(150001,"male"))
            return new Tuple2<>(tup._2(), new Tuple2<>(tup._1(), tup._3()));
        })
                .groupByKey()
                .foreach((VoidFunction<Tuple2<String, Iterable<Tuple2<Integer, String>>>>) tup -> {
                    //获取大区信息
                    String area = tup._1;
                    System.out.print(area + ":");
    ...
    private static void flatMapOp(JavaSparkContext sc) {
        ...
        dataRDD.flatMap((FlatMapFunction<String, String>) line -> {
    ...
    private static void filterOp(JavaSparkContext sc) {
        ...
        dataRDD.filter((Function<Integer, Boolean>) i1 -> i1 % 2 == 0)
                .foreach((VoidFunction<Integer>) System.out::println);
    }

    private static void mapOp(JavaSparkContext sc) {
        JavaRDD<Integer> dataRDD = sc.parallelize(Arrays.asList(1, 2, 3, 4, 5));
        dataRDD.map((Function<Integer, Integer>) i1 -> i1 * 2)
                .foreach((VoidFunction<Integer>) System.out::println);
    }
